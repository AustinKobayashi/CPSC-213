...
int exec() {
    ...
    static const void* insOpCodeJumpTable[16] = { &&L0, &&L1, &&L2, &&L3, &&L4, &&L6,
        &&L7, &&L8, &&L9, &&L10, &&L11, &&L12, &&L13, &&L14, &&L15, &&DEFAULT1};
    
    static const void* insOp0JumpTable[16] = { &&J0, &&J1, &&J2, &&J3, &&J4, &&J5, &&J6,
        &&J7, &&J15, &&DEFAULT2};
    
    if(insOpCode < 0 || insOpCode == 5 || insOpCode > 15) goto DEFAULT1; 
...
L6: // ALU ................... 6-sd
    ...
    if(insOp0 < 0 || (insOp0 > 8 && insOp0 < 15) || insOp0 > 15) goto DEFAULT2;
    if(insOp0 == 15) goto J15;
...
L13: // j *o(rr) .............. droo
    addr = insOp0 + (((unsigned short) insOpImm) << 2);
    pc =  mem [addr] << 24 | mem [addr+1] << 16 | mem [addr+2] << 8 | mem [addr+3];
    goto CONT;
    
L14: // j*(rr,ri,4) ............. eri-
    addr = insOp0 + (insOp1 << 2);
    pc =  mem [addr] << 24 | mem [addr+1] << 16 | mem [addr+2] << 8 | mem [addr+3];
    goto CONT;
